...
func kInversePairs(n, k int) int {
	...
	//          为啥呢? 当k=i  时, j-i+k   = j,   对应右边j-k, k=0的情况
	//                 当k=i-1时, j-i+i-1 = j-1, 对应右边j-k, k=1的情况. 依此类推
	// f[i][j-1]和f[i][j]之间的关系为
	// f[i][j] = f[i][j-1]-f[i-1][j-i]+f[i-1][j]
	//		∵			f[i][j]   -> sum(f[i-1][j-(k+1)]) - f[i-1][j-i](k=i) + f[i-1][j](k=0), k ∈[1,i] ①
	//		又 			f[i][j-1] -> sum(f[i-1][j-1-k]), k∈[0,i-1] ②
	//		②-①可得	f[i][j] - f[i][j-1] = f[i-1][j] - f[i-1][j-i]
	//		∴       	f[i][j] = f[i][j-1] + f[i-1][j] - f[i-1][j-i]
	// 此时, f[i]只和f[i-1]相关, 所以可以进行压缩
	// f[i]需要先计算f[i-1], f[][j]需要先计算f[][j-i] 所以[i,j]都是正向迭代
