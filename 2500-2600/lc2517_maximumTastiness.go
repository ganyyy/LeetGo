...
func maximumTastiness(price []int, k int) int {
	...
	/*
		...
		如果我们把这个最小绝对差记为 x，那么我们可以得到以下(k-1)组不等式：
	...
	*/
	...
	return sort.Search((price[len(price)-1]-price[0])/(k-1), func(interval int) bool {
		interval++ // 二分最小的 f(interval+1) < k，从而知道最大的 f(interval) >= k
		cnt, pre := 1, price[0]
		// 双指针, 找到所有间隔大于 interval 的糖果并计数
		...
		// 如果数量少了, 就意味着 interval 太大了, 需要往左偏(d减小); 否则就是往右偏(d增大)
		return cnt < k
	})
}
