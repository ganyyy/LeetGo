...
func Constructor(n int, parent []int) TreeAncestor {
	...
	// 节点i的第2^j个祖先, i代表的是节点的编号, j代表的是第X个祖先
	// 我们需要存储的父节点分别是: 0,1,2,4,8,16 ..., 所以5w个节点的最大层级是16
	// 如果要查找的是二的整数次幂的父节点, 可以直接找到数组的最高位1定位
	// 那么: 如何应对非二的整数次幂的父节点呢?
	// 这里面就存在了状态转移问题.
	// 比如第3个父节点, 其二进制表示为 0b11, 表示的是节点的第8个父节点
	// 第一位不为0, 则跳转到父节点, 此时就变成了查找父节点的第二个父节点(2*4=8)
	// 比如第6个父节点, 其二进制表示为 0b110, 表示的是节点的第64个父节点
	// 第一位是0, 不需要操作, 第二位不为零, 则先跳转到第二个父节点, 与之对应的就是第二个父节点的第四个父节点(4*16=64)
